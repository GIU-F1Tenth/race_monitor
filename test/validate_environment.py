...
def check_python_version():
    ...
    if version.major == 3 and version.minor >= 10:
        ...
        print(f"❌ Python {version.major}.{version.minor}.{version.micro} is not supported. Requires Python 3.10+")
        return False
...
def check_evo_submodule():
    ...
    if evo_path.exists() and any(evo_path.iterdir()):
        ...
        print("❌ EVO submodule not initialized. Run: git submodule update --init --recursive")
        return False
...
def check_ros2_packages():
    ...
    for package in ros2_packages:
        try:
            ...
            print(f"❌ {package} - Install with: sudo apt install ros-$ROS_DISTRO-{package.replace('_', '-')}")
            all_good = False
