...
class ResearchTrajectoryEvaluator:
    ...
    def setup_output_directories(self):
        ...
        base_dir = self.config.get('trajectory_output_directory', 'trajectory_evaluation')
    ...
    def _calculate_research_metrics(self, traj: trajectory.PoseTrajectory3D) -> Dict[str, float]:
        ...
        try:
            ...
            if velocities:
                ...
                metrics['velocity_consistency'] = float(np.std(velocities) / np.mean(velocities)) if np.mean(velocities) > 0 else 0
    ...
    def export_research_data(self):
        ...
        for fmt in output_formats:
            if fmt == 'json':
                summary_file = os.path.join(self.experiment_dir, 'exports', f'{self.controller_name}_{self.experiment_id}_summary.json')
                with open(summary_file, 'w') as f:
                    json.dump(summary, f, indent=2)
            
            elif fmt == 'csv':
                # Export lap-by-lap metrics as CSV
                csv_file = os.path.join(self.experiment_dir, 'exports', f'{self.controller_name}_{self.experiment_id}_metrics.csv')
                ...
                try:
                    import pickle
                    pickle_file = os.path.join(self.experiment_dir, 'exports', f'{self.controller_name}_{self.experiment_id}_data.pkl')
